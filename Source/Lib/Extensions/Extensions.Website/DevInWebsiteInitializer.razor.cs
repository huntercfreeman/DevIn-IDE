...
using DevIn.Ide.Wasm.Facts;

namespace DevIn.Website.RazorLib;

public partial class DevInWebsiteInitializer : ComponentBase
{
    [Inject]
    ...
    protected override Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
        	BackgroundTaskService.Continuous_EnqueueGroup(new BackgroundTask(
        		...
        		Do_DevInWebsiteInitializerOnAfterRenderAsync));
        }

        return base.OnAfterRenderAsync(firstRender);
    }
    
    public async ValueTask Do_DevInWebsiteInitializerOnAfterRenderAsync()
    {
    ...
    private async Task WriteFileSystemInMemoryAsync()
    {
        ...
        // This line is also in DevInExtensionsDotNetInitializer,
        // but its duplicated here because the website
        // won't open the first file correctly without this.
        TextEditorHeaderRegistry.UpsertHeader("cs", typeof(DevIn.Extensions.CompilerServices.Displays.TextEditorCompilerServiceHeaderDisplay));
