...
public partial class CompilerServiceEditorDisplay : ComponentBase
{
	...
	private void RecalculateViewModel()
	{
		...
		catch (DevInTextEditorException)
		{
			// Eat this exception
		}
	}
		...
		int indentation = 0)
    {
    	...
    	stringBuilderLocal.Append($"<div class=\"di_compiler-service-editor-indentation-{indentation}\">");
		...
		foreach (var child in node.GetChildList())
		{
			...
			{
				stringBuilderLocal.Append($"<div class=\"di_compiler-service-editor-indentation-{childIndentation}\">");
