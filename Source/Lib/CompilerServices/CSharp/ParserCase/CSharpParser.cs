...
using DevIn.CompilerServices.CSharp.ParserCase.Internals;
    ...
    public static void Parse(CSharpCompilationUnit compilationUnit, CSharpBinder binder, ref CSharpLexerOutput lexerOutput)
    {
        ...
        while (true)
        {
			...
			else if (parserModel.TokenWalker.ConsumeCounter < 0)
			{
				...
				throw new DevInTextEditorException($"parserModel.TokenWalker.ConsumeCounter:{parserModel.TokenWalker.ConsumeCounter} < 0");
			}
