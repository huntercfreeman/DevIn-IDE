...
using DevIn.Extensions.CompilerServices.Utility;
    ...
    */
        ...
        ref CSharpParserModel parserModel)
    {
    	...
    	if (codeBlockOwner.ScopeIndexKey != -1)
    	{
    		...
    		throw new DevInTextEditorException($"{nameof(NewScopeAndBuilderFromOwner)} codeBlockOwner.ScopeIndexKey is NOT '-1'; an infinite loop? _{codeBlockOwner.SyntaxKind}");
    	}
        ...
        CSharpCompilationUnit compilationUnit)
    {
    	if (codeBlockOwner.ScopeIndexKey != -1)
    	{
    		...
    		//       ...When the token walker sets the token index for deferred parsing,
    		//       a new instance of the node ends up being parsed.
    		throw new DevInTextEditorException($"{nameof(NewScopeAndBuilderFromOwner)} codeBlockBuilder.ScopeIndexKey is NOT '-1'; an infinite loop?");
		}    
    	...
    	CSharpCodeBlockBuilder codeBlockBuilder, CSharpCompilationUnit compilationUnit, ref CSharpParserModel parserModel)
    {
    	...
    	if (codeBlockBuilder.CodeBlockOwner.ScopeIndexKey == -1)
    		throw new DevInTextEditorException($"{nameof(SetCurrentScopeAndBuilder)} codeBlockBuilder.CodeBlockBuilder.ScopeIndexKey is '-1'. Invoke {NewScopeAndBuilderFromOwner}?");
