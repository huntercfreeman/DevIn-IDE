...
using DevIn.Extensions.CompilerServices.Displays;
using DevIn.CompilerServices.CSharp.BinderCase;
using DevIn.CompilerServices.CSharp.LexerCase;
using DevIn.CompilerServices.CSharp.ParserCase;

namespace DevIn.CompilerServices.CSharp.CompilerServiceCase;
...
public sealed class CSharpCompilerService : IExtendedCompilerService
{
    ...
    /// <summary>
    /// This overrides the default Blazor component: <see cref="DevIn.TextEditor.RazorLib.TextEditors.Displays.Internals.SymbolDisplay"/>.
    /// It is shown when hovering with the cursor over a <see cref="DevIn.TextEditor.RazorLib.CompilerServices.Syntax.Symbols.ISymbol"/>
    /// (as well other actions will show it).
    ///
    /// If only a small change is necessary, It is recommended to replicate <see cref="DevIn.TextEditor.RazorLib.TextEditors.Displays.Internals.SymbolDisplay"/>
        ...
        if (!foundMatch && symbols.Count != 0)
        {
            foreach (var symbol in symbols)
            {
                    ...
                    cursorPositionIndex < symbol.TextSpan.EndExclusiveIndex)
                {
                    ...
                    viewModelModifier.PersistentState.TooltipViewModel = new(
                        typeof(DevIn.Extensions.CompilerServices.Displays.SymbolDisplay),
                        parameters,
                        relativeCoordinatesOnClick,
                        null,
                        componentData.ContinueRenderingTooltipAsync);
