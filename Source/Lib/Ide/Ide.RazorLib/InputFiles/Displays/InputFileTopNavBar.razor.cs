        ...
        try
        {
            if (!await FileSystemProvider.Directory.ExistsAsync(address).ConfigureAwait(false))
            {
                if (await FileSystemProvider.File.ExistsAsync(address).ConfigureAwait(false))
                    throw new DevInIdeException($"Address provided was a file. Provide a directory instead. {address}");

                throw new DevInIdeException($"Address provided does not exist. {address}");
            }
